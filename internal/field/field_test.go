...
var testData = map[string]struct {
	...
	"All":             {`a, args(b:d=f,c:e=g)`, field.Info{Name: "a", Params: []string{"b", "c"}, Enums: []string{"d", "e"}, Defaults: []string{"f", "g"}, DescArgs: []string{"", ""}}},
	"NameOnly":        {`X`, field.Info{Name: "X"}},
	"NameOnly2":       {`joe,`, field.Info{Name: "joe"}},
	"NameNull":        {`joe,nullable`, field.Info{Name: "joe", Nullable: true}},
	"Params0":         {`,args()`, field.Info{Params: []string{}, Enums: []string{}, Defaults: []string{}, DescArgs: []string{}}},
	"Params1":         {`,args(a)`, field.Info{Params: []string{"a"}, Enums: []string{""}, Defaults: []string{""}, DescArgs: []string{""}}},
	"Params2":         {`,args(abc,d)`, field.Info{Params: []string{"abc", "d"}, Enums: []string{"", ""}, Defaults: []string{"", ""}, DescArgs: []string{"", ""}}},
	"Params3":         {`,args(abc,"d e",f)`, field.Info{Params: []string{"abc", `"d e"`, "f"}, Enums: []string{"", "", ""}, Defaults: []string{"", "", ""}, DescArgs: []string{"", "", ""}}},
	"ParamsSpaced":    {`,args( a , bcd , efg )`, field.Info{Params: []string{"a", "bcd", "efg"}, Enums: []string{"", "", ""}, Defaults: []string{"", "", ""}, DescArgs: []string{"", "", ""}}},
	"Defaults1":       {`,args(one=1,2)`, field.Info{Params: []string{"one", "2"}, Enums: []string{"", ""}, Defaults: []string{"1", ""}, DescArgs: []string{"", ""}}},
	"Defaults2":       {`,args(one=1,two="number two")`, field.Info{Params: []string{"one", "two"}, Enums: []string{"", ""}, Defaults: []string{"1", `"number two"`}, DescArgs: []string{"", ""}}},
	"Defaults3":       {`,args(list=[1,2,4],obj={a:1, b:"two"})`, field.Info{Params: []string{"list", "obj"}, Enums: []string{"", ""}, Defaults: []string{"[1,2,4]", `{a:1, b:"two"}`}, DescArgs: []string{"", ""}}},
	...
	"EnumParams":      {`,args(height, unit:Unit)`, field.Info{Params: []string{"height", "unit"}, Enums: []string{"", "Unit"}, Defaults: []string{"", ""}, DescArgs: []string{"", ""}}},
	"EnumParams2":     {`,args(h, w, unit:Unit = FOOT)`, field.Info{Params: []string{"h", "w", "unit"}, Enums: []string{"", "", "Unit"}, Defaults: []string{"", "", "FOOT"}, DescArgs: []string{"", "", ""}}},
	...
	"ArgDesc":         {`,args(a#desc)`, field.Info{Params: []string{"a"}, Enums: []string{""}, Defaults: []string{""}, DescArgs: []string{"desc"}}},

	"AllOptions": {`a:b,,args(c:d=e#f,g=h#i i i i),nullable,subscript=h#d #d`, // Note that this is invalid at a higher level as you can't use both "args" and "subscript" options together
		field.Info{Name: "a", GQLTypeName: "b", Params: []string{"c", "g"}, Enums: []string{"d", ""}, Defaults: []string{"e", "h"},
			DescArgs: []string{"f", "i i i i"}, Nullable: true, Subscript: "h", Description: "d #d"}},
}
